...
mod tests {
    ...
    #[tokio::test]
    async fn unified_exec_releases_process_id_when_additional_permissions_invalid() {
        use crate::features::Feature;
        use crate::protocol::AskForApproval;
        use crate::sandboxing::SandboxPermissions;
        use crate::turn_diff_tracker::TurnDiffTracker;
        ...
        session.features.enable(Feature::RequestPermission);
        turn_context_raw.approval_policy = AskForApproval::OnRequest;
        let session = Arc::new(session);
        let turn_context = Arc::new(turn_context_raw);
        let tracker = Arc::new(tokio::sync::Mutex::new(TurnDiffTracker::new()));
        ...
        assert!(output.contains("fs_write contains an empty path"));

        let next_process_id = session
            .services
            .unified_exec_manager
            .allocate_process_id()
            .await;
        pretty_assertions::assert_eq!(next_process_id, "1000");
        session
            .services
            .unified_exec_manager
            .release_process_id(&next_process_id)
            .await;
    }
}
