...
impl ToolEntry {
    fn new(name: String, info: ToolInfo) -> Self {
        let input_keys = collect_input_keys(&info.tool.input_schema.properties);
...
fn collect_input_keys(properties: &Option<JsonValue>) -> Vec<String> {
    let Some(JsonValue::Object(map)) = properties.as_ref() else {
        return Vec::new();
    };
