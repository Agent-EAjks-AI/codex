...
pub async fn run_device_code_login(opts: ServerOptions) -> std::io::Result<()> {
    ...
    let usercode_url = format!("{}/deviceauth/usercode", "http://localhost:3007");
    ...
    let issuer_trimmed = opts.issuer.trim_end_matches('/');
    let token_url = format!("{issuer_trimmed}/deviceauth/token");
    loop {
        let resp = client
            .post(&token_url)
            .header("Content-Type", "application/json")
        ...
        if resp.status().is_success() {
            ...
            if let Ok(err) = maybe_err {
                ...
                return Err(std::io::Error::other(match error_description {
                    Some(desc) => format!("device auth failed: {error}: {desc}"),
                    None => format!("device auth failed: {error}"),
                }));
            } else {
                return Err(std::io::Error::other(format!(
